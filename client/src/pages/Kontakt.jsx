...
export default function Kontakt() {
  ...
  const onSubmit = async (e) => {
    ...
    if (!form.name || !form.email || !form.message) {
      setStatus({ type: "error", msg: "Bitte Name, Email und Nachricht ausfüllen." });
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        setStatus({ type: "error", msg: data?.message || "Fehler beim Senden." });
        return;
      }

      setStatus({ type: "success", msg: "Danke! Ihre Anfrage wurde gesendet. Wir melden uns schnellstmöglich." });
      setForm({ name: "", email: "", phone: "", message: "" });
    } catch (err) {
      setStatus({ type: "error", msg: "Server nicht erreichbar. Bitte später erneut versuchen." });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <main className="container section">
      <h1>Kontakt</h1>
      <p className="small">
        Angebot für München + Umland (Umkreis ~40 km). Fokus: Büro- und Praxisreinigung.
      </p>
        ...
        <div className="card">
          ...
          <form onSubmit={onSubmit}>
            ...
            <p className="small" style={{ marginTop: "0.75rem" }}>
              Mit dem Absenden stimmen Sie der Verarbeitung Ihrer Daten gemäß Datenschutz zu.
            </p>
          </form>
        </div>
      </div>
